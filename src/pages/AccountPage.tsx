...
export default function AccountPage() {
  ...
  // shopping list: everything in the library still missing the key flag
  // (archived items are out — they're set aside, not on the wishlist)
  const toBuyCount = items.filter((i) => !i.owned && !i.archived).length
  ...
  return (
    <div className="pb-6">
      ...
      {/* to buy — the mirror of Owned, same box style, right below it */}
      <Link
        to="/tobuy"
        className="mx-4 mt-3 flex items-center gap-3 rounded-2xl border border-line bg-card p-4 transition-colors hover:border-accent/50"
      >
