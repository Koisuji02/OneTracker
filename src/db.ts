...
/**
 * Last time the user actually TOUCHED an item: the most recent unit marked
 * (episodes/chapters carry their own timestamp) or the last single-media
 * interaction, with the add date as the floor. Every "Continue" section and
 * the home-screen widget order by this one number, so any mark — a first watch
 * or a rewatch — floats its item to the top of the list it lives in.
 */
export function unitActivity(eps: WatchedEpisode[]): Map<string, number> {
  const m = new Map<string, number>()
  for (const e of eps) m.set(e.itemId, Math.max(m.get(e.itemId) ?? 0, e.watchedAt))
  return m
}

/** Activity instant of one item, given the map built by `unitActivity`. */
export function lastActivity(item: LibraryItem, units?: Map<string, number>): number {
  return Math.max(units?.get(item.id) ?? 0, item.lastReadAt ?? 0, item.addedAt ?? 0)
}

/** Comparator for Continue lists: most recently touched first. */
export function byActivity(units?: Map<string, number>) {
  return (a: LibraryItem, b: LibraryItem) => lastActivity(b, units) - lastActivity(a, units)
}
...
export async function markRewatchUnit(
  ...
  // a rewatch tap is a real interaction: stamp it so the Continue lists (which
  // order by activity) float the item back to the top, exactly like a first
  // watch does. Status is deliberately NOT recomputed — the item stays
  // `completed`, it's the rewatch round that's in progress.
  await db.items.update(item.id, { lastReadAt: Date.now() })
}
